...
const ProductCarousel = () => {
  ...
  return (
    <div className="mb-4 xl:block lg:block md:block mt-20 text-black">
     {isLoading ? null : error ?(
        <Message variant='danger' >
         {error?.data?.Message  || error.message}
        </Message>
     ): <Slider {...settings} className="xl:w-[50rem] lg:w-[50rem] md:-[56rem]
     sm:w-[40rem] sm:block">
        {products.map(
            ({
              image,
              ...
              <div key={_id}>
                <img
                  src={image}
                  alt={name}
                  className="w-[400px] rounded-lg object-cover h-[300px] ms-40 "
                />
